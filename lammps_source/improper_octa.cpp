...
void ImproperOcta::compute(int eflag, int vflag)
{
 ...
 Jordan = ve/v0[type];
 //printf("Jordan! %f\n", Jordan);
 if (Jordan < 0.0) error->all(FLERR,"Negative volume!");
...
/// replaced here
  ...
  double mu_one = utils::numeric(FLERR,arg[1],false,lmp);
  double lamda_one = utils::numeric(FLERR,arg[2],false,lmp);
  double v0_one = utils::numeric(FLERR,arg[3],false,lmp);
  int ele5_one  = utils::numeric(FLERR,arg[4],false,lmp);
  int ele6_one  = utils::numeric(FLERR,arg[5],false,lmp);
  int ele7_one  = utils::numeric(FLERR,arg[6],false,lmp);
  int ele8_one  = utils::numeric(FLERR,arg[7],false,lmp);
////
  int count = 0;
